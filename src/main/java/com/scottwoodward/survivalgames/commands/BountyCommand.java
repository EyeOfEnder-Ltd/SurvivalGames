				...
				if(target != null){
					if(PlayerManager.getInstance().isInGame(sender.getName())){
						...
						if(!game.getAllSpectators().contains(sender.getName())){
							sender.sendMessage(ChatColor.YELLOW + "You must be spectating a match to issue a bounty");
						}else if(game.getAllPlayers().contains(target.getName())){
							if(game.getAllPlayers().size() >= 5){
									...
									try{
										...
										if(amount > 0){
											...
											if(currentPoints >= amount){
												...
												game.addBounty(target.getName(), amount);
											}else{
								...
								sender.sendMessage(ChatColor.YELLOW + "There must be five active players remaining to issue a bounty");
							}
						}else{
							sender.sendMessage(ChatColor.YELLOW + target.getName() + " is not an active player in the same lobby as you.");
						}
					}else{
						sender.sendMessage(ChatColor.YELLOW + "You must be spectating a game to issue a bounty.");
					}
				}else{
					sender.sendMessage(ChatColor.YELLOW + "Player " + args[0] + " not found.");
				}
			}else{
				sender.sendMessage(ChatColor.YELLOW + "Usage: /bounty <Player Name> <Amount>");
			}			
		}else{
			sender.sendMessage(ChatColor.YELLOW + "You must be a player to issue a bounty.");
		}
	}
}
